  ...
  return (
    <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-lg shadow-lg px-2 sm:px-4 md:px-8 border-b border-lime-100">
      <div className="max-w-7xl mx-auto flex items-center justify-between py-2 sm:py-3 md:py-5">
        ...
        >
          <span className={`block w-7 h-0.5 bg-emerald-600 mb-1 rounded transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
          <span className={`block w-7 h-0.5 bg-emerald-600 mb-1 rounded transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}></span>
          <span className={`block w-7 h-0.5 bg-emerald-600 rounded transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
        </button>
        {/* Navigation */}
        <nav
        ...
        >
          ...
          <Link
            href="/register"
            className="flex items-center justify-center gap-2 border border-emerald-400 text-emerald-700 font-bold px-5 py-2 rounded-full bg-white hover:bg-emerald-100 hover:text-emerald-900 transition-all duration-200 shadow-sm md:ml-2 mt-4 md:mt-0 w-full md:w-auto"
